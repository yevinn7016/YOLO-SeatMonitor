...
import { useHealthQuery, useLayoutQuery, useSaveLayoutMutation } from '@/hooks/use-seat-api'
...
export function SeatSettingsPage() {
  ...
  const imageError = failedFrameUrl === frameUrl
  ...
  const updateSelectedSeatLabel = (field: keyof SeatLabelParts, value: string) => {
    if (!selectedRegion || !selectedLabelParts) return
    const nextParts = { ...selectedLabelParts, [field]: value }
    ...
    <div className="mx-auto max-w-7xl">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_360px]">
        <section className="surface overflow-hidden">
          ...
          <div className="relative grid aspect-video place-items-center overflow-hidden bg-slate-900">
            {currentStep === 2 && capturedImageUrl ? (
              ...
              <img src={capturedImageUrl} alt="캡처된 카메라 화면" className="size-full object-contain" />
            ) : cameraConnected && !imageError ? (
              <img src={frameUrl} alt="최신 카메라 프레임" className="size-full object-contain" onLoad={() => setFailedFrameUrl(null)} onError={() => setFailedFrameUrl(frameUrl)} />
            ) : (
        ...
        <aside className="surface p-6">
          ...
          {currentStep === 2 && selectedRegion && selectedLabelParts && <div className="mt-4 rounded-xl border border-brand-100 bg-brand-50/60 p-4"><div className="flex items-center gap-2 text-xs font-bold text-slate-700"><Pencil size={14} />선택한 좌석 편집</div><div className="mt-3 grid grid-cols-3 gap-2"><label className="text-[10px] text-slate-500">테이블<input aria-label="테이블 번호" value={selectedLabelParts.table} onChange={(event) => updateSelectedSeatLabel('table', event.target.value)} className="mt-1 h-9 w-full rounded-lg border border-slate-200 bg-white px-2 text-center text-xs font-semibold uppercase outline-none focus:border-brand-500" /></label><label className="text-[10px] text-slate-500">행<input aria-label="좌석 행" value={selectedLabelParts.row} onChange={(event) => updateSelectedSeatLabel('row', event.target.value)} className="mt-1 h-9 w-full rounded-lg border border-slate-200 bg-white px-2 text-center text-xs font-semibold uppercase outline-none focus:border-brand-500" /></label><label className="text-[10px] text-slate-500">번호<input aria-label="좌석 번호" value={selectedLabelParts.number} onChange={(event) => updateSelectedSeatLabel('number', event.target.value)} className="mt-1 h-9 w-full rounded-lg border border-slate-200 bg-white px-2 text-center text-xs font-semibold outline-none focus:border-brand-500" /></label></div><div className="mt-3 rounded-lg bg-white px-3 py-2 text-center text-xs font-bold text-brand-700">{selectedRegion.label}</div><p className="mt-2 text-[11px] leading-4 text-slate-400">테이블-행-번호 순서로 좌석 ID가 자동 생성됩니다.</p></div>}
